...
import { Worker_type } from '../schema/worker.schema';
...
export class WorkerTypeService {
  ...
  async findOne(workerType_id: string): Promise<Worker_type> {
    try {
      const workerType = await this.workerTypeModel.findOne({ _id: workerType_id });
      if (!workerType) {
      ...
      throw new InternalServerErrorException(`Error finding workerType: ${error.message}`);
    }
  }

  
  async update(workerType_id: string, updateWorkerTypeDto: UpdateWorkerTypeDto) {
    const workerType = await this.workerTypeModel.findOne({ _id: workerType_id });
    if (!workerType) {
      throw new BadRequestException('Tipo de trabajador no encontrado');
    }
  ...
  async remove(workerType_id: string) {
    const workerType = await this.workerTypeModel.findOneAndDelete({ _id: workerType_id });
    if (!workerType) {
      throw new BadRequestException('Tipo de trabajador no encontrado');
    }
